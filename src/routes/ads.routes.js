...
/**
 * @route   POST /api/ads
 * @desc    Create new ad (status: draft). Order: upload -> cloudinary -> validateCreateAd -> createAd.
 * @access  Private
 * @middleware protect - JWT authentication required
 * @middleware uploadImages - Handle multipart/form-data
 * @middleware uploadToCloudinary - Upload to Cloudinary (required)
 * @middleware validateCreateAd - Allowlist + body validation (details allowed)
 */
router.post(
  '/',
  protect,
